...
router.post('/createnewroom', async (req, res) => {
    ...
    if (!req.isAuthenticated()) {
        ...
        // Parsing date to UTC
        var starttime = moment(req.body.starttime, ["h:mm A"]).format("HH:mm");
        var endtime = moment(req.body.endtime, ["h:mm A"]).format("HH:mm");

        // var utcStart = new moment(starttime, "HH:mm").utc();
        // var utcEnd = new moment(endtime, "HH:mm").utc();

        var utcStart = new moment(req.body.date + " " +starttime, "DD-MM-YYYY HH:mm").utc();
        var utcEnd = new moment(req.body.date + " " +endtime, "DD-MM-YYYY HH:mm").utc();
